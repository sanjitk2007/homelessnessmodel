...
actual_2020 = int(total_row['Year 2020'].values[0].replace(',', ''))
mse_linear = mean_squared_error([actual_2020], predicted_2020_linear.astype(int))
mae_linear = mean_absolute_error([actual_2020], predicted_2020_linear.astype(int))
results['Linear Regression'] = {
    'Prediction': predicted_2020_linear[0],
    'MSE': mse_linear,
    'MAE': mae_linear
}
print(f"Linear Regression - Prediction: {predicted_2020_linear[0]:.0f}, MSE: {mse_linear:.2f}, MAE: {mae_linear:.2f}")

# SVM
svm_model = SVR(kernel='linear')
svm_model.fit(X, y)
predicted_2020_svm = svm_model.predict([[2020]])
mse_svm = mean_squared_error([actual_2020], predicted_2020_svm.astype(int))
mae_svm = mean_absolute_error([actual_2020], predicted_2020_svm.astype(int))
results['SVM'] = {
    'Prediction': predicted_2020_svm[0],
    'MSE': mse_svm,
    'MAE': mae_svm
}
print(f"SVM - Prediction: {predicted_2020_svm[0]:.0f}, MSE: {mse_svm:.2f}, MAE: {mae_svm:.2f}")

# Random Forest
rf_model = RandomForestRegressor(n_estimators=100, random_state=42)
rf_model.fit(X, y)
predicted_2020_rf = rf_model.predict([[2020]])
mse_rf = mean_squared_error([actual_2020], predicted_2020_rf.astype(int))
mae_rf = mean_absolute_error([actual_2020], predicted_2020_rf.astype(int))
results['Random Forest'] = {
    'Prediction': predicted_2020_rf[0],
    'MSE': mse_rf,
    'MAE': mae_rf
}
print(f"Random Forest - Prediction: {predicted_2020_rf[0]:.0f}, MSE: {mse_rf:.2f}, MAE: {mae_rf:.2f}")
...
print(f"MLP - Prediction: {predicted_2020_mlp:.0f}, MSE: {mse_mlp:.2f}, MAE: {mae_mlp:.2f}")
...
print(f"LSTM - Prediction: {predicted_2020_lstm:.0f}, MSE: {mse_lstm:.2f}, MAE: {mae_lstm:.2f}")
