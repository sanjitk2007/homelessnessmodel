...
def predict_homeless_population_2022(state):
    years, homeless = prepare_data(state)
    ...
    actual_2022 = int(homeless_data[homeless_data['State'] == state]['Year 2022'].values[0].replace(',', ''))
    mse_linear = mean_squared_error([actual_2022], predicted_2022_linear)
    mae_linear = mean_absolute_error([actual_2022], predicted_2022_linear)
    results['Linear Regression'] = {
        'Prediction': predicted_2022_linear[0],
        'MSE': mse_linear,
        'MAE': mae_linear
    }
    print(f"{state} - Linear Regression - Prediction: {predicted_2022_linear[0]:.0f}, MSE: {mse_linear:.2f}, MAE: {mae_linear:.2f}")

    # SVM
    svm_model = SVR(kernel='linear')
    svm_model.fit(X, y)
    predicted_2022_svm = svm_model.predict([[2022]])
    mse_svm = mean_squared_error([actual_2022], predicted_2022_svm)
    mae_svm = mean_absolute_error([actual_2022], predicted_2022_svm)
    results['SVM'] = {
        'Prediction': predicted_2022_svm[0],
        'MSE': mse_svm,
        'MAE': mae_svm
    }
    print(f"{state} - SVM - Prediction: {predicted_2022_svm[0]:.0f}, MSE: {mse_svm:.2f}, MAE: {mae_svm:.2f}")

    # Random Forest
    rf_model = RandomForestRegressor(n_estimators=100, random_state=42)
    rf_model.fit(X, y)
    predicted_2022_rf = rf_model.predict([[2022]])
    mse_rf = mean_squared_error([actual_2022], predicted_2022_rf)
    mae_rf = mean_absolute_error([actual_2022], predicted_2022_rf)
    results['Random Forest'] = {
        'Prediction': predicted_2022_rf[0],
        'MSE': mse_rf,
        'MAE': mae_rf
    }
    print(f"{state} - Random Forest - Prediction: {predicted_2022_rf[0]:.0f}, MSE: {mse_rf:.2f}, MAE: {mae_rf:.2f}")
    ...
    print(f"{state} - MLP - Prediction: {predicted_2022_mlp:.0f}, MSE: {mse_mlp:.2f}, MAE: {mae_mlp:.2f}")
    ...
    print(f"{state} - LSTM - Prediction: {predicted_2022_lstm:.0f}, MSE: {mse_lstm:.2f}, MAE: {mae_lstm:.2f}")

    return results

state_results_2022 = {}
for state in states:
    state_results_2022[state] = predict_homeless_population_2022(state)

for state, results in state_results_2022.items():
